...
top_terms = 'top_terms_twitter.txt'
...
def SVCModel(data, ngram):
    ...
    vectorizer = CountVectorizer(tokenizer = spacy_tokenizer, ngram_range=ngram)
...
def MultinomialNBModel(data, ngram):
    """
    Uses Multinomial Naive Bayes Classifer to train
    """
    tfidf = TfidfVectorizer(sublinear_tf=True, min_df=5, norm='l2', ngram_range=ngram, stop_words='english')

    # apply CountVectorizer(), TfidfTransformer(), (transforms) and final estimator(classifier)
    pipe = Pipeline([('vect', CountVectorizer(tokenizer = spacy_tokenizer, ngram_range=ngrams)),
...
def BernoulliNBModel(data, ngram):
    """
    Uses Multinomial Naive Bayes Classifer to train
    """
    tfidf = TfidfVectorizer(sublinear_tf=True, min_df=5, norm='l2', ngram_range=ngram, stop_words='english')
    ...
    # apply CountVectorizer(), TfidfTransformer(), (transforms) and final estimator(classifier)
    pipe = Pipeline([('vect', CountVectorizer(tokenizer = spacy_tokenizer, ngram_range=ngram)),
                    ('tfidf', TfidfTransformer()),
...
def ComplementNBModel(data, ngram):
    """
    Uses Multinomial Naive Bayes Classifer to train
    """
    tfidf = TfidfVectorizer(sublinear_tf=True, min_df=5, norm='l2', ngram_range=ngram, stop_words='english')
...
def SGDModel(data, ngram):
    ...
    pipe.fit(data.Sentences, data.Label)
    print_top20_words("SGD with TFIDF", vectorizer, pipe)
    return pipe

def LSVCModel(data, ngram):
    """
    Uses Multinomial Support Vector Machine to train
    """
    #transformers using Spacy
    ...
    vectorizer = CountVectorizer(tokenizer = spacy_tokenizer, ngram_range=ngram)
    # vectorizer = TfidfVectorizer(sublinear_tf=True, min_df=5, norm='l2', ngram_range=ngram, stop_words='english')
    ...
    # Load sample data
    pipe.fit(data.Sentences, data.Label)
    print_top20_words('LSVC with BOW', vectorizer, pipe)

    return pipe

def LogisticRegressionModel(data, ngram):
    """
    Uses Multinomial Support Vector Machine to train
    """
    #transformers using Spacy
    ...
    # Load sample data
    pipe.fit(data.Sentences, data.Label)
    print_top20_words('LR with BOW', vectorizer, pipe)
    return pipe
